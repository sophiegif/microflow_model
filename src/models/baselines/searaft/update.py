...
from src.models.baselines.searaft.layer import ConvNextBlock
...
class BasicMotionEncoderNoCorr(nn.Module):
    def __init__(self, args, dim=128):
        super(BasicMotionEncoderNoCorr, self).__init__()
        self.convf1 = nn.Conv2d(2, dim, 7, padding=3)
        self.convf2 = nn.Conv2d(dim, dim//2, 3, padding=1)
        self.conv = nn.Conv2d(dim//2, dim-2, 3, padding=1)
    ...
    def forward(self, flow):
        # flow 32, 2, 32, 32
        flo = F.relu(self.convf1(flow))  # 32, 128, 32, 32
        flo = F.relu(self.convf2(flo))  # 32, 64, 32, 32
        out = F.relu(self.conv(flo))  # 32, 126, 32, 32
        return torch.cat([out, flow], dim=1)
...
class BasicUpdateBlock(nn.Module):
    def __init__(self, args, hdim=128, cdim=128):
        ...
        self.refine = []
        for i in range(args.searaft_num_blocks):
...
class BasicUpdateBlockNoCorr(nn.Module):
    def __init__(self, args, hdim=128, cdim=128):
        ...
        self.args = args
        self.encoder = BasicMotionEncoderNoCorr(args, dim=cdim)
        self.refine = []
        for i in range(args.searaft_num_blocks):
            self.refine.append(ConvNextBlock(2*cdim+hdim, hdim))
        self.refine = nn.ModuleList(self.refine)

    def forward(self, net, inp, flow):
        motion_features = self.encoder(flow)
        inp = torch.cat([inp, motion_features], dim=1)
        for blk in self.refine:
            net = blk(torch.cat([net, inp], dim=1))
        return net